...
def get_coor():
    ...
    while (True):
        user_input = input(
            "Please enter coordinates (ex: 3a   or   A6    or   7A) ? ")
        try:
            ...
            coor[1] = num
            print(coor)
            # if str(OriginalCoor[0]).isalpha() and not str(OriginalCoor[0]).isupper():
                #coor[0] = ord(OriginalCoor[0])-ord('a')
                #coor[1] = int(OriginalCoor[1]) - 1
            # elif not str(OriginalCoor[1]).isupper():
                #coor[0] = ord(OriginalCoor[1])-ord('a')
                #coor[1] = int(OriginalCoor[0]) - 1

            # elif str(OriginalCoor[0]).isalpha() and str(OriginalCoor[0]).isupper():
                # coor[0]=ord(OriginalCoor[0])-ord('A')
                # coor[1]=int(OriginalCoor[1])-1
            # elif str(OriginalCoor[1]).isupper():
                # coor[0]=ord(OriginalCoor[1])-ord('A')
                # coor[1]=int(OriginalCoor[0])-1
